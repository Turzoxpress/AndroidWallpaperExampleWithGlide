...
/**
 * Created by Turzo on 4/29/2017.
 */

public class FacebookOpen extends AppCompatActivity {

    private Button sendstyle,likeus;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.facebook_open);

        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setHomeButtonEnabled(true);

        sendstyle = (Button)findViewById(R.id.sendstyle);
        likeus = (Button)findViewById(R.id.likeus);

        sendstyle.setOnClickListener(new View.OnClickListener(

        ) {
            @Override
            public void onClick(View v) {

                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/Nail-Polish-Styles-461018570912731/"));
